/**
 * search_eu_implementations — Search EU directives/regulations that have Malawian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_malawian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as malawian_statute_count
  ...
  if (input.has_malawian_implementation) {
    sql += ' HAVING malawian_statute_count > 0';
  }
