/**
 * Tool registry for Malawi Law MCP Server.
...
import { getMalawianImplementations, type GetMalawianImplementationsInput } from './get-malawian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Malawi Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Malawian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Malawian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section/article) from a Malawian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Malawian legal citation against the database — zero-hallucination check. ' +
      ...
      'Supports formats: "Section 25, Data Protection Act 2019", "s 25, DPA 2019", "Article 31, Constitution of Malawi 2010".',
    inputSchema: {
      type: 'object',
      properties: {
        citation: {
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Malawian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Malawi?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Malawian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Malawian statute or provision is currently in force, amended, repealed, partially suspended, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Malawian statute aligns with or references. ' +
      'Malawi is not an EU member but several Malawian laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Malawian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_malawian_implementations',
    description:
      'Find all Malawian statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Malawian statutes. ' +
      'Note: Malawi implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Malawian implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_malawian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Malawian aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Malawian statute. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_malawian_implementations':
          result = await getMalawianImplementations(db, args as unknown as GetMalawianImplementationsInput);
          break;
