/**
 * get_malawian_implementations — Find Malawian statutes implementing a specific EU directive/regulation
...
export interface GetMalawianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface MalawianImplementationResult {
  document_id: string;
...
export async function getMalawianImplementations(
  db: InstanceType<typeof Database>,
  input: GetMalawianImplementationsInput,
): Promise<ToolResponse<MalawianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as MalawianImplementationResult[];
