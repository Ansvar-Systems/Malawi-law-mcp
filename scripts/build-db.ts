#!/usr/bin/env tsx
/**
 * Database builder for Malawi Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Malawi Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Malawian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'MW');
    insertMeta.run('source', 'malawilii.org');
