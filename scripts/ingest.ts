#!/usr/bin/env tsx
/**
 * Malawi Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from malawilii.org (Akoma Ntoso HTML).
 *
 * Features:
 ...
 *
 * Data source: malawilii.org (National Council for Law Reporting)
...
import { parseMalawiLawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Malawi Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('====================================================\n');
  console.log(`  Source: malawilii.org (National Council for Law Reporting)`);
  ...
  for (const law of acts) {
    ...
    try {
      ...
      const parsed = parseMalawiLawHtml(html, act);
  ...
  console.log(`\n  Source:      malawilii.org (Akoma Ntoso HTML)`);
